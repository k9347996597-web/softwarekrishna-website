function Cart(localStorageKey) {  //create a function to create multiple objects 
  const cart = {
    cartItems: undefined,


    loadFromStorage() {
      this.cartItems = JSON.parse(localStorage.getItem(localStorageKey));//retrive data
      if (!this.cartItems) {
        this.cartItems = [{
    ...
    saveToStorage() {
      localStorage.setItem(localStorageKey, JSON.stringify(this.cartItems));//save the data
    },
